...
export default function Randevular() {
    ...
    const [formValues, setFormValues] = useState({
        province: '34', letters: '', digits: '', customer: '', phone: '', service: '', time: new Date().toISOString().slice(0, 16)
    });
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            setFormValues({ province: '34', letters: '', digits: '', customer: '', phone: '', service: '', time: new Date().toISOString().slice(0, 16) });
            setError('');
            setErrorFields([]);
        } catch (saveError) {
    ...
    return (
        <div className="animate-in fade-in duration-500">
            ...
            <div className="bg-[var(--bg-card)] p-6 rounded-2xl border border-[var(--border-strong)]/30 shadow-2xl mb-8 animate-in slide-in-from-top-4 duration-700">
                    ...
                    <div className="space-y-5">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input type="text" placeholder="Ahmet Yilmaz" value={formValues.customer} onChange={(e) => setFormValues(prev => ({ ...prev, customer: e.target.value }))} className={`w-full p-4 bg-[var(--bg-main)] border rounded-xl text-[var(--text-primary)] outline-none transition-all placeholder:text-gray-700 font-sans ${errorFields.includes('customer') ? 'border-red-500 shadow-[0_0_10px_rgba(236,77,55,0.1)]' : 'border-[var(--border-strong)] focus:border-[var(--accent)]'}`} />
                            </div>
                            <div className="space-y-1">
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <input type="tel" placeholder="5XX XXX XX XX" value={formatPhoneDisplay(formValues.phone)} onChange={(e) => setFormValues(prev => ({ ...prev, phone: e.target.value.replace(/\D/g, '').slice(0, 10) }))} className={`flex-1 p-4 bg-[var(--bg-main)] border rounded-xl text-[var(--text-primary)] outline-none transition-all placeholder:text-gray-700 font-sans ${errorFields.includes('phone') ? 'border-red-500 shadow-[0_0_10px_rgba(236,77,55,0.1)]' : 'border-[var(--border-strong)] focus:border-[var(--accent)]'}`} />
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="space-y-5 flex flex-col">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="space-y-1">
                                ...
                                <input type="datetime-local" min={now} value={formValues.time} onChange={(e) => setFormValues(prev => ({ ...prev, time: e.target.value }))} className={`w-full p-4 bg-[var(--bg-main)] border rounded-xl text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-all ${errorFields.includes('time') ? 'border-red-500 shadow-[0_0_10px_rgba(236,77,55,0.1)]' : 'border-[var(--border-strong)]'}`} />
                            </div>
                            <div className="space-y-1">
                                <label className="text-[10px] text-gray-500 font-bold uppercase ml-1 tracking-wider">Yapilacak Islem *</label>
                                <input type="text" placeholder="Orn: Yag Degisimi" value={formValues.service} onChange={(e) => setFormValues(prev => ({ ...prev, service: e.target.value }))} className={`w-full p-4 bg-[var(--bg-main)] border rounded-xl text-[var(--text-primary)] focus:border-[var(--accent)] outline-none transition-all placeholder:text-gray-700 ${errorFields.includes('service') ? 'border-red-500 shadow-[0_0_10px_rgba(236,77,55,0.1)]' : 'border-[var(--border-strong)]'}`} />
                            </div>
                        </div>
                    ...
                    <div key={app.id} className={`group p-5 rounded-2xl bg-[var(--bg-card)] border-l-8 transition-all hover:scale-[1.02] shadow-xl ${app.type === 'green' ? 'border-[var(--success)]' : 'border-[var(--danger)]'}`}>
                        ...
                        <div className="flex gap-2 mt-3 pt-3 border-t border-[var(--border-strong)]/30">
                            {app.status === 'ONAY BEKLİYOR' ? (
                                ...
                                <button onClick={() => { deleteAppointment(app.id); navigate('/vehicle-entry', { state: { plate: app.plate, service: app.service, customer: app.customer, phone: app.phone } }); }} className="flex-1 py-2 bg-[var(--border-strong)] text-[var(--text-primary)] font-black rounded-lg hover:bg-[var(--accent)] hover:text-white transition-all text-xs uppercase tracking-widest shadow-md">IS EMRI AC</button>
                            )}
